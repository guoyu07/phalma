...
class Sword extends Engine
{
    ...
    protected function _render($file, $vars = [])
    {
        if (PHP_EOL !== "\n") {
            ...
            if ($realPath !== str_replace('\\', '/', $file)) {
                trigger_error("File name ($realPath) case mismatch for .$file", E_USER_ERROR);
            }
        }

        if (strpos($file, APP_PATH) === 0) {
            $_compiledFile = DATA_PATH . '/sword' . str_replace(APP_PATH, '', $file);
        } else {
            $_compiledFile = DATA_PATH . '/sword/mixed/' . md5($file);
        }
        ...
        if (!file_exists($_compiledFile) || filemtime($file) > filemtime($_compiledFile)) {
            ...
            if (!is_dir($dir) && !@mkdir($dir, 0755, true) && !is_dir($dir)) {
                throw new Exception("create `:$dir` directory failed: :last_error_message");
            }
            if (file_put_contents($_compiledFile, $this->compileString(file_get_contents($file)), LOCK_EX) === false) {
                throw new Exception("write compiled sword `:$_compiledFile` failed: :last_error_message");
            }
        }

        $vars['renderer'] = $this->_view;
        $vars['di'] = new Component();
