...
class Debugger extends Component implements DebuggerInterface
{
    ...
    public function _eventHandlerPeek($source, $data, $event)
    {
        if ($event === 'logger:log') {
            ...
            if (count($this->_sql_prepared) <= $this->_sql_prepared_max) {
                $preparedSQL = $source->getRealSQLStatement();
            ...
            if (count($this->_sql_executed) <= $this->_sql_executed_max) {
                $this->_sql_executed[] = [
                    'prepared' => $source->getRealSQLStatement(),
                    'bind' => $source->getSQLVariables(),
                    'emulated' => $this->sqlDePreparer->deprepare($source->getRealSQLStatement(), $source->getSQLVariables())
                ];
            }
    ...
    public function start()
    {
        if (isset($_GET['_debugger']) && preg_match('#^[a-zA-Z0-9_/]+\.html$#', $_GET['_debugger'])) {
            $file = DATA_PATH . '/debugger' . $_GET['_debugger'];
            if (file_exists($file)) {
    ...
    public function output($template = 'Default')
    {
        ...
        $data['configure'] = isset($this->configure) ? (array)$this->configure : [];
...
//        }
        ...
        ob_start();
        $file = ROOT_PATH . '/ManaPHP/Debugger/Template/' . $template . '.phtml';
        extract($data);
        require $file;
        return ob_get_clean();
    }
    ...
    {
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'ApacheBench')) {
            return '';
        }

        $file = date('/ymd/His_') . $this->random->base58(32) . '.html';

        $file_path = DATA_PATH . '/debugger' . $file;
        /** @noinspection MkdirRaceConditionInspection */
        @mkdir(dirname($file_path), 0755, true);
        file_put_contents($file_path, $this->output($template));
